...
void PmergeMe::fordJohnsonSortWrapper() {
    // Tüm sıralama işlemleri öncesinde ilk hali için Before bir kez yazdırılıyor
    std::cout << "Before: ";
    this->vectorPrinter(); // veya intDequePrinter() - yalnızca birini kullanmak yeterli

    // Vektör için sıralama ve zaman ölçümü
    clock_t start = clock();
    this->fordJohnsonSort(this->vector_data, 0, this->vector_data.size() - 1);
    clock_t stop = clock();
    double duration = ((stop - start) / (double)CLOCKS_PER_SEC) * 1000000;
    
    // Deque için sıralama ve zaman ölçümü
    start = clock();
    this->fordJohnsonSort(this->deque_data, 0, this->deque_data.size() - 1);
    stop = clock();
    double duration2 = ((stop - start) / (double)CLOCKS_PER_SEC) * 1000000;

    // Sıralama sonrası için After bir kez yazdırılıyor
    std::cout << "After: ";
    this->vectorPrinter(); // veya intDequePrinter() - yalnızca birini kullanmak yeterli

    // Zaman ölçüm sonuçları için tek satırlık çıktı
